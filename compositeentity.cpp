...
void CompositeEntity::updateChildren(bool paused, double deltaTimeMilliseconds, int direction) {
    std::vector<Entity*>::iterator it;
    for (it = children.begin(); it != children.end(); it++) {
        (*it)->update(paused, deltaTimeMilliseconds, direction);
    }
}

//void CompositeEntity::setVelocity(int new_velocity){
//    std::vector<Entity*>::iterator it;
//    for (it = children.begin(); it != children.end(); it++) {
//        (*it)->setVelocity(new_velocity);
//    }
//}
